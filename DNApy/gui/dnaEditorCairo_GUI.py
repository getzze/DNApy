...
from . import DNApyBaseClass, DNApyBaseDrawingClass, featureedit_GUI, output_GUI,colcol
...
class TextEdit(DNApyBaseDrawingClass):
	...
	def __init__(self, parent, id, genbank):
		...
		# get data
		self.dna            = self.genbank.gb.GetDNA()
		self.cdna           = self.genbank.gb.GetDNA()
...
#       self.SetSize(wx.Size(w,self.minHeight)) # prevents missfomration on resize
	...
	def drawFeatures(self):
		features    = self.genbank.gb.get_all_feature_positions()
		paths       = {} # (color, path)
...
######################################################
	...
	def OnMotion(self, event):
		if event.Dragging() and event.LeftIsDown():
			oldSel = self.genbank.dna_selection
...
####################################################################
	...
	def set_cursor_position(self, pointer=False):
		...
		self.genbank.cursor_position = pointer
	...
	def moveCursor(self, offset, linejump = False):
		...
		index = self.PositionPointer
		topology = self.genbank.gb.gbfile['locus']['topology']

		length = len(self.genbank.gb.GetDNA())
	...
	def OnKeyPress(self, evt):
		...
		if key in [97, 65, 116, 84, 99, 67, 103, 71]: # [a, A, t, T, c, C, g, G']
			...
			if finish != -1: # if a selection, delete it, then paste
				self.genbank.gb.Delete(start+1, finish, visible=False)
				self.PositionPointer = start
			if shift == True:
				self.genbank.gb.Paste(index, chr(key).upper())
			elif shift == False:
				self.genbank.gb.Paste(index, chr(key).lower())
			...
			if finish != -1: # if a selection, delete it
				self.genbank.gb.Delete(start+1, finish)
				...
				self.genbank.gb.Delete(index-1, index-1)
			...
			start, finish, null = self.get_selection()
			if finish != -1: # if a selection, delete it
				self.genbank.gb.Delete(start, finish)
				self.PositionPointer = start
			else:
				self.genbank.gb.Delete(index, index)
			self.set_dna_selection() # remove selection
			self.update_ownUI()
			self.update_globalUI()
...
################ genbank methods ###############
	def select_all(self):
		...
		start   = 1
		finish  = len(self.genbank.gb.GetDNA())+1 # TODO figure the coorect way to select. Write it in some dokumentation
	...
	def uppercase(self):
		...
		if finish == -1:
			...
			self.genbank.gb.Upper(start, finish)
			self.update_ownUI()
			self.set_dna_selection()
	...
	def lowercase(self):
		...
		if finish == -1:
			...
			self.genbank.gb.Lower(start, finish)
			self.update_ownUI()
			self.set_dna_selection()
	...
	def reverse_complement_selection(self):
		...
		if finish == -1:
			...
			self.genbank.gb.RCselection(start, finish)
			self.update_ownUI()
			self.update_globalUI()
			self.set_dna_selection()
	...
	def cut(self):
		...
		if finish == -1:
			...
			self.genbank.gb.Cut(start, finish)
			self.set_cursor_position(start)
			self.update_ownUI()
			self.update_globalUI()
			self.set_dna_selection()
	...
	def cut_reverse_complement(self):
		...
		if finish == -1:
			...
			self.genbank.gb.CutRC(start, finish)
			self.update_ownUI()
			self.update_globalUI()
			self.set_dna_selection()
	...
	def paste(self):
		...
		if finish == -1:
			...
			self.genbank.gb.Delete(start, finish, visible=False)
			self.set_cursor_position(start)
		self.genbank.gb.RichPaste(self.genbank.cursor_position)
		self.update_ownUI()
		self.update_globalUI()
	...
	def paste_reverse_complement(self):
		...
		if finish == -1:
			...
			self.genbank.gb.Delete(start, finish, visible=False)
			self.set_cursor_position(start)
		self.genbank.gb.PasteRC(self.genbank.cursor_position)
		self.update_ownUI()
		self.update_globalUI()
		self.set_dna_selection() # reset selection
	...
	def copy(self):
		...
		if finish == -1:
			...
			#self.genbank.gb.Copy(start, finish)

			# try the new copy:
			self.genbank.gb.RichCopy(start, finish)
	...
	def copy_reverse_complement(self):
		...
		if finish == -1:
			...
			self.genbank.gb.CopyRC(start, finish)
...
#       tabtext = str(self.stc.GetPageText(self.stc.GetSelection()))
	...
	def translate_selection(self):
		...
		if finish == -1:
			...
			DNA = self.genbank.gb.GetDNA(start, finish)
...
#update this one...
	def translate_feature(self):
		...
		feature = self.genbank.gb.allgbfeatures[2]
		DNA = self.genbank.gb.getdnaforgbfeature(feature[4])
		protein = dna.Translate(DNA)
...
#           #which feature corresponds to this pos?
			Feature = self.genbank.gb.get_featurename_for_pos(mposition)
