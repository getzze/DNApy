...
from .. import dna
from .. import genbank as genbank_module
from .. import enzyme

#GUI components
from . import dnaEditorCairo_GUI, plasmid_GUI, genbank_GUI, enzyme_GUI, output_GUI, mixed_base_codons_GUI, DNApyBaseClass
...
variables_file = os.path.join(files['default_dir'], "variables")   ##path to the file of the global variables
settings_file = os.path.join(files['default_dir'], "settings")   ##path to the file of the global settings
execfile(variables_file) #gets all the pre-assigned variables
execfile(settings_file) #gets all the pre-assigned settings

class GenBank(object):
	"""General object to handle genbank data
...
class DNAedit(DNApyBaseClass):
	...
	def __init__(self, parent, id, genbank):
		...
		self.dnaview = dnaEditorCairo_GUI.TextEdit(self.scroll, id=wx.ID_ANY, genbank=self.genbank) # This generates 8 OnSize draw events
		...
		self.parent = parent
		self.plasmidview = plasmid_GUI.PlasmidView2(splitter0, -1, genbank=self.genbank)
	...
	def onChangeText(self, text):
		...
		'''
		#self.genbankrestriction_sites =  self.genbankrestrictionEnzymes.getEnzymes(self.genbankrestriction_sites)
...
class DNApy(wx.Frame):
	...
	def __init__(self, parent, id, title):
		...
		self.genbank = GenBank()
		self.genbank.gb = genbank_module.gbobject() #make new gb in panel
		...
		self.genbank.dna_selection       = (1, -1, -1)       # variable for storing current DNA selection
		self.genbank.cursor_position     = 1             # to save cursor position


		self.genbank.feature_selection   = False #variable for storing current feature selection
		self.genbank.search_hits         = []
		self.genbank.gb.fileName         = ''

		# initialise the retriction enzyme:
		self.genbank.restrictionEnzymes      = enzyme.initRestriction(self.genbank)
		# save the selected restriktion enzymes
		self.genbank.restriction_sites       = {}

		#build the UI using the pre-defined class
		self.DNApy = DNAedit(self, -1, genbank=self.genbank)
	...
	def do_layout(self):
		...
		try:
			...
			logger.exception("Could not add toolbar.")
			print("Could not add toolbar: {}".format(e))   # DEBUG: Remove line when the type of exception is identified
		self.SetSizer(sizer)
...
################ file functions #################
	...
	def new_file(self, evt):
		'''Create new gb file'''
		if not self.fileopen: #if no file is open, make blank gb file
...
#           self.page_change("")
		...
		else: #if a file IS open, make a new window
	...
	def open_file(self, evt):
		...
		self.genbank.gb.fileName = dlg.GetFilename()
		all_path = dlg.GetPath()
		dire = dlg.GetDirectory()
		dlg.Destroy()

		if(self.genbank.gb.fileName == None or self.genbank.gb.fileName == "" ):
			return 1



		name, extension = self.genbank.gb.fileName.split('.')
		if extension.lower() == 'gb':
			self.genbank.gb = genbank_module.gbobject(all_path) #make a genbank object and read file

			self.SetTitle(' - '.join([self.genbank.gb.fileName , __title__]))
			if self.genbank.gb.clutter == True: #if tags from ApE or Vector NTI is found in file
	...
	def save_as_file(self, evt):
		...
		filepath = self.genbank.gb.GetFilepath()
		...
		if (fileName == None or fileName == ""):
			...
			self.genbank.gb.fileName = fileName
			if self.genbank.gb.fileName[-3:].lower() != '.gb': #make sure it has gb file ending
				all_path += '.gb'
				self.genbank.gb.fileName += '.gb'
			#try:
			self.genbank.gb.SetFilepath(all_path)
			self.save_file("")
			self.SetTitle(' - '.join([self.genbank.gb.fileName , __title__]))
			#except:
			#   error_window(7, self)
...
#       return self.genbank.feature_selection #returns integer
...
#       num_features = len(self.genbank.gb.get_all_features()) #number of features already present
...
#       self.genbank.feature_selection = int(copy.copy(index))
#       #add logic to find first and last position for feature and make DNA selection match.
#       #print('Feature "%s" selected') % (self.get_feature_label(self.genbank.feature_selection))
...
#####################################
	...
	def Undo(self, evt):
		self.genbank.gb.Undo()
		self.update_globalUI()


	def Redo(self, evt):
		self.genbank.gb.Redo()
		self.update_globalUI()
...
######################################
	...
	def SelectEnzymes(self, evt):
		...
		'''
		#launch the dialog
		dlg = enzyme_GUI.EnzymeSelectorDialog(None, 'Enzyme Selector', self.genbank.restriction_sites,self.genbank.restrictionEnzymes)
		dlg.Center()
		...
		if res == wx.ID_OK:
			self.genbank.restriction_sites = dlg.GetSelection()
	...
	def digestDna(self, evt):
		...
		'''
		if self.genbank.gb.gbfile["dna"]:
			#launch the dialog
			dlg = enzyme_GUI.EnzymeDigestionDialog(self, 'digestion', self.genbank.restriction_sites, self.genbank.restrictionEnzymes)
			dlg.Center()
			res = dlg.ShowModal() #alternatively, if main window should still be accessible use dlg.Show()
...
##########################################
	...
	def update_ownUI(self):
		...
		print('filever index', self.genbank.gb.get_file_version_index())
		if self.genbank.gb.get_file_version_index() <= 0: #if there are no undos available
			self.frame_1_toolbar.EnableTool(513, False)
		else:
			self.frame_1_toolbar.EnableTool(513, True)

		if self.genbank.gb.get_file_version_index() >= len(self.genbank.gb.file_versions)-1: #if there are no redos available
	...
	def update_globalUI(self):
		...
		self.genbank.restrictionEnzymes.reloadEnzymes()
...
######################################
	...
	def list_features(self, evt):
		...
		self.make_outputpopup()
#       tabtext = str(self.genbank.gbviewer.GetPageText(self.genbank.gbviewer.GetSelection()))
		tabtext = 'Replace!'
		self.output.write('%s | List features\n' % tabtext, 'File')
		featurelist = self.genbank.gb.ListFeatures()
	...
	def view_genbank(self, evt):
		...
		genbankview = genbank_GUI.MyPanel(dlg, style=wx.VSCROLL|wx.HSCROLL, genbank=self.genbank) #put the genbank view panel inside
...
######### Toolbar and Menu ############
	...
	def __generate_toolbar(self):
		...
		#New Document
		self.frame_1_toolbar.AddLabelTool(500, "New Document", wx.Bitmap(os.path.join(ICONS_DIR, "new.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'New File', "New File") #last one is the one displayed in status bar
		wx.EVT_TOOL(self, 500, self.new_file)
		#Open File
		self.frame_1_toolbar.AddLabelTool(501, "Open File", wx.Bitmap(os.path.join(ICONS_DIR, "open.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Load File', 'Load File')
		wx.EVT_TOOL(self, 501, self.open_file)
		#Save current file
		self.frame_1_toolbar.AddLabelTool(502, "Save current file", wx.Bitmap(os.path.join(ICONS_DIR, "save.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Save File', 'Save File')
		wx.EVT_TOOL(self, 502, self.save_file)
		#Save as
		self.frame_1_toolbar.AddLabelTool(503, "Save as", wx.Bitmap(os.path.join(ICONS_DIR, "saveas.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Save File As', 'Save File As')
		wx.EVT_TOOL(self, 503, self.save_as_file)
		#cut
		self.frame_1_toolbar.AddLabelTool(504, "cut", wx.Bitmap(os.path.join(ICONS_DIR, "cut.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'cut', 'cut')
		wx.EVT_TOOL(self, 504, self.cut)
		#copy
		self.frame_1_toolbar.AddLabelTool(505, "copy", wx.Bitmap(os.path.join(ICONS_DIR, "copy.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'copy', 'copy')
		wx.EVT_TOOL(self, 505, self.copy)
		#paste
		self.frame_1_toolbar.AddLabelTool(506, "paste", wx.Bitmap(os.path.join(ICONS_DIR, "paste.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'paste', 'paste')
		wx.EVT_TOOL(self, 506, self.paste)
		#Undo
		self.frame_1_toolbar.AddLabelTool(513, "Undo", wx.Bitmap(os.path.join(ICONS_DIR, "undo.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Undo', 'Undo')
		wx.EVT_TOOL(self, 513, self.Undo)
		#Redo
		self.frame_1_toolbar.AddLabelTool(514, "Redo", wx.Bitmap(os.path.join(ICONS_DIR, "redo.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Redo', 'Redo')
...
#       wx.EVT_TOOL(self, 510, self.print_setup)
		...
		#enzyme selector
		self.frame_1_toolbar.AddLabelTool(515, "Restriction Enzymes", wx.Bitmap(os.path.join(ICONS_DIR, "restrictionEnzyme.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Restriction Enzymes', 'Restriction Enzymes')
	...
	def add_search_tools(self, typeof):
		...
		try:
			...
			logger.exception("Could not make feature list.")
			print("Could not make feature list: {}".format(e))   # DEBUG: Remove line when the type of exception is identified
			pass
		...
		if typeof == 'Find':
			#find previous
			self.frame_2_toolbar.AddLabelTool(507, "Find previous", wx.Bitmap(os.path.join(ICONS_DIR, "up.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Find previous', 'Find previous')
			wx.EVT_TOOL(self, 507, self.find_previous)
			#find next
			self.frame_2_toolbar.AddLabelTool(509, "Find next", wx.Bitmap(os.path.join(ICONS_DIR, "down.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Find next', 'Find next')
			wx.EVT_TOOL(self, 509, self.find_next)
			#search glass
			self.frame_2_toolbar.AddLabelTool(604, "Find", wx.Bitmap(os.path.join(ICONS_DIR, "search.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Find', 'Find')
			wx.EVT_TOOL(self, 604, self.find)
		elif typeof == 'Mutate':
			self.frame_2_toolbar.AddLabelTool(604, "Mutate", wx.Bitmap(os.path.join(ICONS_DIR, "up.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, 'Mutate', 'Mutate')
	...
	def find(self, evt):
		...
		if searchtype == 'Nucleotide':
			self.genbank.search_hits = self.genbank.gb.FindNucleotide(searchstring, searchframe)
		elif searchtype == 'Amino Acid':
			self.genbank.search_hits = self.genbank.gb.FindAminoAcid(searchstring, searchframe)
		elif searchtype == 'Feature':
			self.genbank.search_hits = self.genbank.gb.FindFeature(searchstring)

		#update the dna selection to the first search hit, if there is at least one.
		if len(self.genbank.search_hits) > 0:
			self.set_dna_selection(self.genbank.search_hits[0])
		self.update_globalUI()
	...
	def find_previous(self, evt):
		...
		for i in range(len(self.genbank.search_hits)):
			if start < self.genbank.search_hits[0][0]:
				self.set_dna_selection(self.genbank.search_hits[-1])
				break
			elif start <= self.genbank.search_hits[i][0]:
				self.set_dna_selection(self.genbank.search_hits[i-1])
				break
		self.update_globalUI()
	...
	def find_next(self, evt):
		...
		for i in range(len(self.genbank.search_hits)):
			if start < self.genbank.search_hits[i][0]:
				self.set_dna_selection(self.genbank.search_hits[i])
				break
			elif i == len(self.genbank.search_hits)-1:
				self.set_dna_selection(self.genbank.search_hits[0])
				break
		self.update_globalUI()
	...
	def mutate(self, evt):
		...
		if mutationtype == 'Nucleotide':
			self.genbank.gb.mutate('D', mutationframe, mutation_list)
		elif mutationtype == 'Amino Acid':
			self.genbank.gb.mutate('A', mutationframe, mutation_list)
